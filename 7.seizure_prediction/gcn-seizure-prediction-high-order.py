...
from models.tree_gnn import HighOrderTreeSequentialGCNModel
...
from dataset.tree_dataset import HighOrderGCNTreeDataset
...
device = 'cpu'
dataset = HighOrderGCNTreeDataset(tree_records_base_path, dataset_types, device=device)
...
def custom_collate(batch):
    ...
    batched_graphs = Batch.from_data_list(graphs).to(device) # list of list of graphs
...
model = HighOrderTreeSequentialGCNModel(
    gcn_hidden_dim=256,
    lstm_hidden_dim=128,
...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        graphs, labels = batch['graphs'], batch['label'] # list of list of graphs, list of labels
        output = model(graphs)
        if args.binary:
            labels[labels == 2] = 0
        # Compute the loss
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader):
            graphs, labels = batch['graphs'], batch['label']
            output = model(graphs)
            if args.binary:
                labels[labels == 2] = 0
            loss = criterion(output, labels)
