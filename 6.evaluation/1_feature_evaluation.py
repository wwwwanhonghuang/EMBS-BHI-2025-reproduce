...
def sample_and_compute_mutual_info(featre_records, key, sample_size=1000, n_iterations=10, evaluation_items=['normal', 'seizures']):
    mi_values = []
    for _ in range(n_iterations):
        sampled_data = featre_records[key].sample(n=sample_size, random_state=np.random.randint(1000))
...
def read_feature_csv_files(feature_files):
    ...
    for file in feature_files:
        ...
        if file.find("normal") >= 0:
            ...
            raise KeyError
        featre_records[entry].append(pd.read_csv(os.path.join('/data1', file)))
...
def load_and_sample(featre_records, feature_name, sample_size=1000):
    ...
    sampled_data = {
        'normal': featre_records['normal'][featre_records['normal']['Key'] == feature_name].sample(n=sample_size),
        'seizures': featre_records['seizures'][featre_records['seizures']['Key'] == feature_name].sample(n=sample_size) if 'seizures' in featre_records else [],
        'preepileptic': featre_records['preepileptic'][featre_records['preepileptic']['Key'] == feature_name].sample(n=sample_size) if 'preepileptic' in featre_records else []
    }

    return sampled_data

def plot_kl_divergence_bar(kl_values):
    ...
    plt.bar(keys, means, yerr=stds, capsize=5, color='blue', label="KL Divergence")
    
    # Labeling the plot
    plt.xlabel('Feature Name')
    plt.ylabel('KL Divergence')
    plt.title('Normal-Seizure Feature KL Divergence Plot')
...
for comparision_pair in itertools.combinations(args.kl_comparision_list, 2):
    ...
    for feature_name in tqdm(common_feature_names):
        ...
        kl_values = {feature_name: sample_and_compute_kl(featre_records, feature_name, sample_size=1000, n_iterations=10, comparision_pair=comparision_pair) for key in feature_name}

    if save_KL:
        np.save(os.path.join(output_folder, f"kl_values_{comparision_pair[0]}_{comparision_pair[1]}.npy", kl_values, allow_pickle = True))

    plot_kl_divergence_bar(kl_values)
    plt.savefig(os.path.joing(output_folder, 
...
for feature_name in tqdm(common_feature_names):
    ...
    mutual_information_values = \
        {feature_name: sample_and_compute_mutual_info(featre_records, feature_name, sample_size=1000, n_iterations=10, evaluatio_pair=args.mi_evaluation_list) for key in feature_name}
